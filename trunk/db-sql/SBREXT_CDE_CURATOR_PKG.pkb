CREATE OR REPLACE PACKAGE BODY sbrext_cde_curator_pkg AS
        ...
        CURSOR c_count_cds( p_pv_idseq permissible_values_view.pv_idseq%TYPE ) IS
            SELECT COUNT( cvv.cd_idseq )
             ...
             WHERE pvv.pv_idseq = p_pv_idseq
               AND pvv.short_meaning = vmv.short_meaning
               AND vmv.short_meaning = cvv.short_meaning;
    BEGIN
    ...
    BEGIN
        SELECT MIN( cdv.preferred_name )
         ...
         WHERE vmv.short_meaning = p_short_meaning
           AND vmv.short_meaning = cvv.short_meaning
...
-- AND d.de_idseq    = hc.ac_idseq (+)
 ...
 FROM sbr.data_elements_view               de
    ...
    , sbr.value_meanings_lov_view          vm
    , sbr.value_domains_view               vd
 WHERE '
                || v_con_name
                || '
  ...
  AND de.vd_idseq = vd.vd_idseq
        ...
        v_sql := v_select || v_from || ' WHERE ' || v_where || '  ORDER BY upper(long_name)';   -- 03-Mar-2004, W. Ver Hoef
...
AND   vd.vd_idseq   = vc.vd_idseq    (+) ';   -- 31-Mar-2003, W. Ver Hoef added vc join
 ...
 WHERE '
   ...
   AND propv.prop_idseq = decv.prop_idseq
        ...
        v_sql := v_sql || v_from || ' WHERE ' || v_where || '  ORDER BY   upper(long_name )';
 ...
 AND cvm.short_meaning = cpv.short_meaning
 ...
 AND cvp.vd_IDSEQ = vdv.vd_IDSEQ
        ...
        v_sql := v_select || v_from || ' WHERE ' || v_where || '  ORDER BY   upper(vd.long_name )';   -- 03-Mar-2004, W. Ver Hoef
    ...
    BEGIN
        ...
        OPEN p_vv_cur FOR
            SELECT   VALUE
               ...
               WHERE d.vd_idseq = vp.vd_idseq AND vp.pv_idseq = pv.pv_idseq AND d.de_idseq = p_de_idseq
   ...
   AND        cs.conte_idseq = cx.conte_idseq ';
    ...
    BEGIN
        ...
        OPEN p_valuemeaning_cur FOR
            SELECT DISTINCT v.short_meaning
                           ,v.description
                           ,v.comments
                       FROM value_meanings_lov_view v
                           ,cd_vms_view c
                      WHERE v.short_meaning = c.short_meaning(+)
                            AND NVL( c.cd_idseq, '1' ) = NVL( p_cd_idseq, NVL( c.cd_idseq, '1' ))
                   ORDER BY UPPER( v.short_meaning );
    END;
 ...
 AND CDR.CONDR_IDSEQ = OCV.CONDR_IDSEQ
        ...
        v_sql := v_sql || v_from || ' WHERE ' || v_where || '  ORDER BY   upper(oc.long_name)';
 ...
 AND CDR.CONDR_IDSEQ = propv.CONDR_IDSEQ
        ...
        v_sql := v_sql || v_from || ' WHERE ' || v_where || '  ORDER BY   upper(prop.long_name)';
 ...
 ,vm.condr_idseq
 ,SBREXT_CDE_CURATOR_PKG.get_one_cd_name(p.short_meaning) cd_name';
        v_from := '
 FROM
 ...
 ,value_meanings_lov_view  vm';   -- 16-Feb-2004, W. Ver Hoef added
        ...
        IF p_search_string IS NOT NULL THEN
            v_where :=
                   v_where
                || '
        AND (UPPER(p.short_meaning) LIKE UPPER(NVL('''
                || v_search_string
                || ''',p.short_meaning))
...
--      AND        d.de_idseq = r.ac_idseq(+)   eliminiated 07-Jul-2003, W. Ver Hoef
    ...
    PROCEDURE search_cd(
       ...
       ,p_con_name                 IN       VARCHAR2 DEFAULT NULL   -- 6-DEC-2005, S. Hegde (TT1780)
       ,p_con_idseq                IN       VARCHAR2 DEFAULT NULL   -- 6-DEC-2005, S. Hegde (TT1780)
       ,p_value_meaning            IN       value_meanings_lov_view.short_meaning%TYPE
 ...
 FROM
    conceptual_domains_view  cd
   ,contexts_view            c';
        v_where := 'cd.conte_idseq = c.conte_idseq ';
 ...
 AND CDR.CONDR_IDSEQ = CDV.CONDR_IDSEQ
    ...
    AND vm.short_meaning = vms.short_meaning
    AND vm.short_meaning = '''
                || v_value_meaning
                || ''' ';
        END IF;
 ...
 FROM
  concepts_view_ext con
 ,contexts_view c';
        v_where := 'con.conte_idseq = c.conte_idseq';
 ...
 FROM sbrext.concepts_view_ext             cn
     ...
     ,sbr.value_meanings_lov_view          cvm
     ,sbr.VALUE_DOMAINS_VIEW               VD
 WHERE vd.vd_idseq = '''
                    || p_vd_idseq
                    || '''
