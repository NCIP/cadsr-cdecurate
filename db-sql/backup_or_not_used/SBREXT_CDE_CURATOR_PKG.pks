CREATE OR REPLACE PACKAGE sbrext_cde_curator_pkg AS
    PROCEDURE set_de(
    ...
    */
    ...
    CURSOR pvvm_search_result IS
        ...
        SELECT pv_idseq
              ...
              ,c.VERSION
              -- added on 9/04/2003
        ,      vm.description vm_description   -- 16-Feb-2004, W. Ver Hoef added
          FROM permissible_values p
              ,cd_vms d
              ,conceptual_domains c
              ,value_meanings_lov vm   -- 16-Feb-2004, W. Ver Hoef added
         WHERE p.short_meaning = d.short_meaning AND d.cd_idseq = c.cd_idseq AND p.short_meaning = vm.short_meaning;
    ...
    CURSOR de_vv_list IS
        SELECT VALUE
          ...
          FROM data_elements_view d
              ,vd_pvs_view vp
              ,permissible_values_view pv
         WHERE d.vd_idseq = vp.vd_idseq AND vp.pv_idseq = pv.pv_idseq;
