...
public class ServiceHolder {
	...
	private ServiceHolder(boolean multiConfig) {
		...
		try {
			if (multiConfig) {
				...
				testConfigFolder_ = new File(System
				...
				for (int i = 0; i < serverConfigs.size(); i++) {
					configs_[i] = createPropertiesObject(serverConfigs.get(i),
					        i, testConfigFolder_);
				}

				if (configs_.length > 0) {
					// clear the log files from the last run.
					File temp = new File(configs_[0]
					        .getProperty("LOG_FILE_LOCATION"),
					        "LexBIG_full_log.txt");
					temp.delete();
					temp = new File(configs_[0]
					        .getProperty("LOG_FILE_LOCATION"),
					        "LexBIG_load_log.txt");
					temp.delete();
				}
	...
	public static Properties createPropertiesObject(
	        TestServerConfig serverInfo, int i, File configFolder) {
		...
		props.setProperty("JAR_FILE_LOCATION",
		        "../../../lgSharedLibraries;../../extLib"); // assumes
		// we
		// are
		// runnign
		// in
		...
		props.setProperty("CONFIG_FILE_LOCATION", PropertiesUtility
		        .locatePropFile("config" + System.getProperty("file.separator")
		                + SystemVariables.CONFIG_FILE_NAME, ServiceHolder.class
		                .getName())); // assumes
		// we
		// are
		// running
		...
		props.setProperty("REGISTRY_FILE", new File(confFolder, "registry.xml")
		        .getAbsolutePath());
		...
		props.setProperty("DB_PREFIX", serverInfo.prefix == null ? ""
		        : serverInfo.prefix);
		props.setProperty("DB_PARAM", serverInfo.param == null ? ""
		        : serverInfo.param);
		props.setProperty("DB_USER", serverInfo.username == null ? ""
		        : serverInfo.username);
		props.setProperty("DB_PASSWORD", serverInfo.password == null ? ""
		        : serverInfo.password);
