...
public class ValueDomainServlet extends CurationServlet {
	...
	private void doEditVDActions() throws Exception
	{
		HttpSession session = m_classReq.getSession();
		//begin GF7680
		String workflowStatus = StringUtil.cleanJavascriptAndHtml( (String) m_classReq.getParameter("selStatus") );
		...
		if (sAction ==null ) sAction ="";
		DataManager.setAttribute(session, "VDPageAction", sAction); // store the page action in attribute
		String sSubAction = StringUtil.cleanJavascriptAndHtml( (String) m_classReq.getParameter("VDAction") );
		DataManager.setAttribute(session, "VDAction", sSubAction);
		...
		{
			GetACSearch serAC = new GetACSearch(m_classReq, m_classRes, this);
			String sField = StringUtil.cleanJavascriptAndHtml( (String) m_classReq.getParameter("pvSortColumn") );
			VD_Bean vd = (VD_Bean) session.getAttribute("m_VD");
...
//					}
	...
	public AC_Bean getSystemName(AC_Bean ac, Vector<EVS_Bean> vParent)
	{
		...
		try
		{
			...
			String selNameType = StringUtil.cleanJavascriptAndHtml( (String) m_classReq.getParameter("rNameConv") );
			...
			else
			{
				...
				String sPrefName = StringUtil.cleanJavascriptAndHtml( (String) m_classReq.getParameter("txPreferredName") );
	...
	private void doRemoveParent(String sPVAction, String vdPage) throws Exception
	{
		...
		if (vParentCon == null)
			...
			String sParentCC = StringUtil.cleanJavascriptAndHtml( (String) m_classReq.getParameter("selectedParentConceptCode") );
			String sParentName = StringUtil.cleanJavascriptAndHtml( (String) m_classReq.getParameter("selectedParentConceptName") );
			String sParentDB = StringUtil.cleanJavascriptAndHtml( (String) m_classReq.getParameter("selectedParentConceptDB") );
...
//				m_VD.setVD_REP_TERM(sLongName);
	...
	public AC_Bean getACNames(EVS_Bean newBean, String nameAct, AC_Bean pageAC)
	{
		...
		if (nameAct.equals("Search") || nameAct.equals("Remove"))
		{
			...
			sPrefName = StringUtil.cleanJavascriptAndHtml( (String) m_classReq.getParameter("txPreferredName") );
			if (selNameType != null && selNameType.equals("USER") && sPrefName != null)
	...
	private void doVDUseSelection(String nameAction)
	{
		try
		{
			...
			if (vRepTerm == null) {
				...
				if (sComp.equals("RepTerm") || sComp.equals("RepQualifier"))
				{
					sSelRow = StringUtil.cleanJavascriptAndHtml( (String) m_classReq.getParameter("selRepRow") );
					...
					if (sSelRow != null && !sSelRow.equals(""))
					{
						...
						// get name value pari
						String sNVP = StringUtil.cleanJavascriptAndHtml( (String) m_classReq.getParameter("nvpConcept") );
	...
	private void doInsertVD() throws Exception
	{
		...
		if (sVDEditAction == null)
			sVDEditAction = "";
		String sAction = StringUtil.cleanJavascriptAndHtml( (String) m_classReq.getParameter("ValidateVDPageAction") );
		// String sMenuAction = (String) session.getAttribute(Session_Data.SESSION_MENU_ACTION);
		// String sButtonPressed = (String) session.getAttribute("LastMenuButtonPressed");
		String sOriginAction = (String) session.getAttribute("originAction");
		if (sAction == null)
...
//			VDBeanSR.setVD_REP_TERM(sRepTerm);
	...
	private void doRemoveBuildingBlocksVD() throws Exception
	{
		...
		if (vRepTerm == null)
			...
			{
				sSelRow = StringUtil.cleanJavascriptAndHtml( (String) m_classReq.getParameter("selRepQRow") );
				if (sSelRow != null && !(sSelRow.equals("")))
				{
	...
	public void doOpenViewPage() throws Exception
	{
		...
		if (acID.equals(""))
			acID = StringUtil.cleanJavascriptAndHtml( m_classReq.getParameter("idseq") );
	...
	public void doViewPageTab() throws Exception{
		String tab = StringUtil.cleanJavascriptAndHtml( m_classReq.getParameter("vdpvstab") );
		String from = StringUtil.cleanJavascriptAndHtml( m_classReq.getParameter("from") );
		String id = StringUtil.cleanJavascriptAndHtml( m_classReq.getParameter("id") );
