...
function SubmitValidate(origin)
{
    ...
    if (isValid == "valid")
    {
        showWaitMessage();  //hourglass();  //GF32723
...
function dojoSubmitNewDECForm() {
    ...
    dojo.xhrPost({
        ...
        load: function(result) {
            ...
            temp = 'CreateDEC.js dojoSubmitNewDECForm: loaded response size = [' + result.length + ']';
            //alert(temp);
            window.console && console.log(temp);
...
function dojoCheckEVSStatus(skipStandardConcept) {
    ...
    if (skipStandardConcept === undefined) {
        skipStandardConcept = false;
    }
    window.console && console.log('2 dojoCheckEVSStatus dojo = [' + dojo + '] skipStandardConcept [' + skipStandardConcept + ']');
    dojo.xhrGet({
        // The URL to request
        url: "jsp/CheckEVSStatus.jsp?skipStandardConcept=" + skipStandardConcept,
        handleAs: "json",
        // The method that handles the request's successful result
        // Handle the response any way you'd like!
        load: function (result) {
