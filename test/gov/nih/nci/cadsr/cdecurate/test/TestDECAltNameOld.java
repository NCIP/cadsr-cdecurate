 ...
 *
 */
public class TestDECAltNameOld
{
...
//    logger.initLogger(logXML);
    ...
    TestDECAltNameOld testdec = new TestDECAltNameOld();    
    //testdec.test();

    testdec.initEasyMock();
...
//	    session.setAttribute(EasyMock.eq(Constants.USER_SELECTED_ALTERNATE_DEF_COMP1), EasyMock.isA(HashMap.class));
...
//		EasyMock.replay(servletContext);
		...
		System.out.println("Final string = >>>" + session.getAttribute("userSelectedDefFinal") + "<<<");
		System.out.println("\n");
  }
...
//		EasyMock.replay(servletContext);
		...
		DECHelper.clearAlternateDefinition(mockHttpSession);
		System.out.println("Final string = >>>" + session.getAttribute("userSelectedDefFinal") + "<<<");
		System.out.println("\n");
  }
