<!-- Copyright (c) 2006 ScenPro, Inc.
    $Header: /cvsshare/content/cvsroot/cdecurate/ui/jsp/jsp/SearchParameters.jsp,v 1.19 2007-01-26 19:30:38 hegdes Exp $
...
<form name="searchParmsForm" method="post" action="../../cdecurate/NCICurationServlet?reqType=searchACs">
 <!-- need this style to keep the table aligned top which is different for crf questions   -->
  <table class="sidebarBGColor"  border="0" width="100%"
      <%if (!sSearchAC.equals("Questions")){%> style="position: relative; top: -28px;"<%}
    ...
    <tr>
      ...
      <!-- include all components for regular search or question search-->
       <%  if (!sMenuAction.equals("searchForCreate")) {%> 
            <select name="listSearchFor" size="1" style="width:172" onChange="doSearchForChange();"
                 onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchParameters'); return false">
              <option value="ClassSchemeItems" <%if(sSearchAC.equals("ClassSchemeItems")){%>selected<%}%>>Class Scheme Items</option>
              <option value="ConceptualDomain" <%if(sSearchAC.equals("ConceptualDomain")){%>selected<%}%>>Conceptual Domain</option>
              <option value="Questions" <%if(sSearchAC.equals("Questions")){%>selected<%}%>>CRF Questions</option>
              <option value="DataElement" <%if(sSearchAC.equals("DataElement")){%>selected<%}%>>Data Element</option>
              <option value="DataElementConcept" <%if(sSearchAC.equals("DataElementConcept")){%>selected<%}%>>Data Element Concept</option>
              <option value="ValueDomain" <%if(sSearchAC.equals("ValueDomain")){%>selected<%}%>>Value Domain</option>
        ...
        <% } else {%>
            <select name="listSearchFor" size="1" style="width:172"
                 onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchParameters'); return false">
              <option value="<%=sSearchAC%>" selected><%=sLongAC%></option>
            </select>
        <% } %>
      </td>
    </tr>
...
<%          if (sSearchAC.equals("DataElement")){%>
              ...
              <option value="docText" <%if(sSearchIn.equals("docText")){%>selected<%}%>>Reference Document Text</option>
<% } %>
...
<%          if ((!sMenuAction.equals("searchForCreate")) && (sSearchAC.equals("DataElement") || sSearchAC.equals("Questions"))) {%>
               <option value="CRFName" <%if(sSearchIn.equals("CRFName")){%>selected<%}%>>Protocol ID/CRF Name</option>
<% } %>
...
<%          if (sSearchAC.equals("DataElement")){%>
              <option value="histID" <%if(sSearchIn.equals("histID")){%>selected<%}%>>Historical CDE_ID</option>
<% } %>
               <!-- include permissible value for Data Element and value domain-->
<%          if (sSearchAC.equals("DataElement") || sSearchAC.equals("ValueDomain")){%>
              <option value="permValue" <%if(sSearchIn.equals("permValue")){%>selected<%}%>>Permissible Values</option>
<% } %>
...
<% if (sSearchIn.equals("docText") && sSearchAC.equals("DataElement") && sUIFilter.equals("advanced")) { %>
    ...
    <tr>
      ...
      <th valign="bottom" align=left >Select Document Types:</th>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td >
...
%>
            <option value="<%=sDoc%>" <%if(vDocs != null && vDocs.contains(sDoc)){%>selected<%}%>><%=sDoc%></option>
<%
          } }
%>
        </select>
...
<% if (sSearchIn.equals("CRFName") && sSearchAC.equals("DataElement")) { %>
    <tr><td height="7" colspan=2 valign="top"></td></tr>   
    <tr>
      <th><div valign="top" align=right><%=item++%>)</div></th>    
      <th valign="bottom" ><div align="left">Enter Protocol ID:</div></th>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td>
        <input type="text" name="protoKeyword" size="24" value="<%=sProtoKeyword%>"
          onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchParameters'); return false">
      </td>
    </tr>
    <tr><td height="7" colspan=2 valign="top"></tr>   
    <tr>
      <th><div valign="top" align=right><%=item++%>)</div></th>    
      <th valign="bottom"  ><div align="left">Enter CRF Name:</div></th>
    </tr>
...
<% } else { %>
    ...
    <tr>
      <th><div valign="top" align=right><%=item++%>)</div></th>    
      <th valign="bottom" ><div align="left">Enter Search Term:</div></th>
    </tr>
<% } %>
<!-- same input box for crf name and other keyword searches -->
    <tr>
      <td>&nbsp;</td>
      <td>
        <input type="text" name="keyword" size="24" value="<%=sLastKeyword%>"
          onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchParameters'); return false">
      </td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td >
        <div align="left" title="The wildcard character, *, expands the search to find a non-exact match.">use * as wildcard</div>
      </td>
    </tr>
          ...
          <% if (sSearchAC.equals("DataElement") || sSearchAC.equals("DataElementConcept") || 
              ...
              <%if (sUIFilter.equals("simple")) {%>
                  <a href="javascript:refreshFilter('advanceFilter');"
									onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchFilters'); return false">Advanced Filter</a>
              <%} else {%> 
                  <a href="javascript:refreshFilter('simpleFilter');"
									onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchFilters'); return false">Simple Filter</a><%}%>
          <%}%>          
        </div>
      </th>
    </tr>
...
<%  if (!sSearchAC.equals("Questions")){ %>
  ...
  <%  if (!sSearchAC.equals("PermissibleValue") && !sSearchAC.equals("ValueMeaning")){%>
    <tr>
      <td>&nbsp;</td>
      <td style="height:20"  valign=bottom><b>Context</b></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td>
        <select name="listMultiContextFilter" size="3" style="width:172" multiple
          onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchParameters'); return false">
        <option value="AllContext"  <%if (vContext.size() == 0 || vContext.contains("AllContext")){%>selected<%}%>>All Contexts</option>
<%
        if (vContextAC != null)
        {
...
%>
            <option value="<%=sContextName%>"  <% if (vContext != null && vContext.contains(sContextName)){%>selected<%}%>><%=sContextName%></option>
<%
          }
        }
%>
...
<%if (sSearchAC.equals("DataElement")) {%>
    <tr>
      <td>&nbsp;</td>
      <td style="height:20"  valign=bottom><b>Context Use</b></td>
    </tr>
    <tr style="height:10">
      <td>&nbsp;</td>
      <td align=left>&nbsp;&nbsp;&nbsp;&nbsp;
          <input type="radio" name="rContextUse" value="OWNED_BY" <%if(sContextUse.equals("OWNED_BY")){%>checked<%}%>
					onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchParameters'); return false">
          Owned By</td>
    </tr>
   <% //if (!sSearchIn.equals("CRFName")) { %>
    <tr style="height:10">
      <td>&nbsp;</td>
      <td align=left>&nbsp;&nbsp;&nbsp;&nbsp;
          <input type="radio" name="rContextUse" value="USED_BY" <%if(sContextUse.equals("USED_BY")){%>checked<%}%>
					onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchParameters'); return false">
          Used By</td>
    </tr>
    <tr style="height:10">
      <td>&nbsp;</td>
      <td align=left>&nbsp;&nbsp;&nbsp;&nbsp;
          <input type="radio" name="rContextUse" value="BOTH" <%if(sContextUse.equals("BOTH")){%>checked<%}%>
					onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchParameters'); return false">
          Owned By/Used By</td>
    </tr>
<%    //} 
    ...
    <tr>
      ...
      <td align=left >
          ...
          <input type="radio" name="rVersion" value="Other" <% if (sVersion.equals("Other")) { %> checked <% } %>
              onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchParameters'); return false">
          <input type="text" name="tVersion" value="<%=txVersion%>" maxlength="5" size="3" style="height:20"
...
<%  if (sSearchAC.equals("ValueDomain"))   { %>
    <tr>
      <td>&nbsp;</td>
      <td style="height:20"  valign=bottom><b>Value Domain Type</b></td>
    </tr>
<!-- check box value domain type -->
    <tr>
      <td>&nbsp;</td>
      <td align=left>&nbsp;&nbsp;&nbsp;&nbsp;
          <input type="checkbox" name="enumBox" value="E" <%if(typeEnum != null && typeEnum.equals("E")){%> checked <%}%>
          onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchParameters'); return false">
          Enumerated</td>       
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td align=left>&nbsp;&nbsp;&nbsp;&nbsp;
          <input type="checkbox" name="nonEnumBox" value="N" <%if(typeNonEnum != null && typeNonEnum.equals("N")){%> checked <%}%>
          onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchParameters'); return false">
          Non-Enumerated</td>
    </tr>
  <%}%>    
...
<%  if (sSearchAC.equals("ClassSchemeItems"))   { %>
    <tr>
      <td>&nbsp;</td>
      <td style="height:20"  valign=bottom>Classification Schemes</td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td >
        <select name="listCSName" size ="1" style="width:172" valign="top"
		      onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchParameters'); return false">
            <option value="AllSchemes" selected>All Schemes</option>
...
<%           }     %>
  ...
  <% if (sSearchAC.equals("PermissibleValue") || sSearchAC.equals("ValueMeaning") || 
    ...
    <tr>
      <td>&nbsp;</td>
      <td style="height:20"  valign=bottom><b>Conceptual Domain</b></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td>
...
<!-- workflow status filter for all ACs except csi, pv, vm -->
  <%   if (!sSearchAC.equals("ClassSchemeItems") && !sSearchAC.equals("PermissibleValue") 
    ...
    <tr>
      <td>&nbsp;</td>
      <td style="height:20"  valign=bottom><b>Workflow Status</b> </td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td>
...
%>
              <option value="<%=sStatusName%>" <%if((vStatus != null) && (vStatus.contains(sStatusName))){%>selected<%}%>><%=sStatusName%></option>
<%
            }
          }
...
<%
          ...
          {
%>
              <option value="DRAFT NEW" selected>DRAFT NEW</option>
<%
          }
%>
          </select>
...
<%  if (sSearchAC.equals("DataElement"))   { %>
    <tr>
      <td>&nbsp;</td>
      <td style="height:20"  valign=bottom><b>Registration Status</b></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td>
        <select name="listRegStatus" size="1" style="width:172"
            ...
            <option value="allReg"  <%if (vRegStatus == null || vRegStatus.size()==0 
              || sRegStatus == null || sRegStatus.equals("") || sRegStatus.equals("allReg")){%>selected<%}%>>All Statuses</option>
<%          if (vRegStatus != null) 
            {            
...
<%   } //endif data element %>
    <tr> 
      <td colspan=2>
        <table>
          <col width=5%><col width=20%><col width=32%><col width=14%><col width=20%>
          <tr>
            <td>&nbsp;</td>
            <td style="height:20" colspan=4 valign=bottom><b>Date Created</b>&nbsp;&nbsp;(MM/DD/YYYY)</td>
          </tr>
          <tr>
            <td>&nbsp;</td>
            <td align=right>From </td>
            <td><input type="text" name="createdFrom" value="<%=sCreatedFrom%>" size="8" readonly
               onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchParameters'); return false"></td>
            <td align=center><a href="javascript:show_calendar('searchParmsForm.createdFrom', null, null, 'MM/DD/YYYY');">
              <img name="Calendar" src="../../cdecurate/Assets/calendarbutton.gif" border=1 width="18" 
                height="18" alt="Calendar" style="vertical-align: top;"></a></td>
            <td align=left><a href="javascript:clearDate('createdFrom');">Clear</a></td>
          </tr>
          <tr>
            <td>&nbsp;</td>
            <td align=right>To </td>
            <td><input type="text" name="createdTo" value="<%=sCreatedTo%>" size="8" readonly
               onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchParameters'); return false"></td>
            <td align=center><a href="javascript:show_calendar('searchParmsForm.createdTo', null, null, 'MM/DD/YYYY');">
              <img name="Calendar" src="../../cdecurate/Assets/calendarbutton.gif" border=1 width="18" height="18" alt="Calendar" style="vertical-align: top">
              </a></td>     
            <td align=left><a href="javascript:clearDate('createdTo');">Clear</a></td>
          </tr> 
<!-- creator filter-->
          <tr>
            <td>&nbsp;</td>
            ...
            <td style="height:20" colspan=4 valign=top>
              <select name="creator" size ="1" style="width:172" valign="top" 
                ...
                <option value="allUsers"  <%if (vUsers == null || vUsers.size()==0 
                  || sCreator == null || sCreator.equals("") || sCreator.equals("allUsers")){%>selected<%}%>>All Creators</option>
<%              if (vUsers != null) 
                {            
...
%>
                  <option value="<%=sUser%>" <%if(sUser.equals(sCreator)){%>selected<%}%>><%=sUserName%></option>
<%
                } }
%>
              </select>
...
<!-- modified date filter-->
          <tr>
            <td>&nbsp;</td>
            <td style="height:20" colspan=4 valign=bottom><b>Date Modified</b>&nbsp;&nbsp;(MM/DD/YYYY)</td>
          </tr>
          <tr>
            <td>&nbsp;</td>
            <td align=right>From </td>
            <td><input type="text" name="modifiedFrom" value="<%=sModifiedFrom%>" size="8"  readonly
               onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchParameters'); return false"></td>
            <td align=center><a href="javascript:show_calendar('searchParmsForm.modifiedFrom', null, null, 'MM/DD/YYYY');">
                <img name="Calendar" src="../../cdecurate/Assets/calendarbutton.gif" border="1" width="18" height="18" 
                alt="Calendar" style="vertical-align: center"></a></td>      
            <td align=left><a href="javascript:clearDate('modifiedFrom');">Clear</a></td>
          </tr>
          <tr>
            <td>&nbsp;</td>
            <td align=right>To </td>
            <td><input type="text" name="modifiedTo" value="<%=sModifiedTo%>" size="8"  readonly
               onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchParameters'); return false"></td>
            <td align=center><a href="javascript:show_calendar('searchParmsForm.modifiedTo', null, null, 'MM/DD/YYYY');">
              <img name="Calendar" src="../../cdecurate/Assets/calendarbutton.gif"  border=1 width="18" height="18" 
                alt="Calendar" style="vertical-align: center"></a></td>
            <td align=left><a href="javascript:clearDate('modifiedTo');">Clear</a></td>
          </tr> 
<!-- modifier filter-->
          <tr>
            <td>&nbsp;</td>
            <td style="height:20" colspan=4 valign=bottom><b>Modifier</b></td>
          </tr>
          <tr>
            <td>&nbsp;</td>
            <td style="height:35" colspan=4 valign=top>
              <select name="modifier" size ="1" style="width:172" valign="top" 
                onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_SearchParameters'); return false">
                <option value="allUsers"  <%if (vUsers == null || vUsers.size()==0 
                  || sModifier == null || sModifier.equals("") || sModifier.equals("allUsers")){%>selected<%}%>>All Modifiers</option>
<%              if (vUsers != null) 
                {            
                  for (int i = 0; vUsers.size()>i; i++)
                  {
                    ...
                    if (vUsersName != null && vUsersName.size()>i)                    
                      sUserName = (String)vUsersName.elementAt(i);
%>
                  <option value="<%=sUser%>" <%if(sUser.equals(sModifier)){%>selected<%}%>><%=sUserName%></option>
<%
                } }
%>
              </select>
    ...
    <tr>
      <td class="dashed-black" colspan=2>
        ...
        <div align="right" valign="bottom">
          <select name="listAttrFilter" size="4" style="width:175" multiple   valign="bottom"
            onHelp = "showHelp('Help_SearchAC.html#searchParmsForm_displayAttributes'); return false">
<%        if (vACAttr != null)
          {
